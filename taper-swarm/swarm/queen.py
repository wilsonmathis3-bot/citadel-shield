...
class Queen:
    ...
    def queue_analysis(self, idea_id, agent_name):
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('''INSERT INTO tasks (idea_id, agent_name, task_type, status)
                     VALUES (?, ?, 'analysis', 'queued')''', (idea_id, agent_name))
        conn.commit()
        conn.close()
    ...
    def get_all_reports(self, idea_id):
        ...
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("SELECT * FROM agent_reports WHERE idea_id = ? ORDER BY created_at DESC", (idea_id,))
    ...
    def calculate_final_score(self, idea_id):
        ...
        score = 0
        weights = {
            'critic': 0.30,    # Skeptic has highest weight
            'banker': 0.25,    # Money matters
            'scout': 0.20,     # Market validation
            'smith': 0.15,     # Technical feasibility
            'shield': 0.05,    # Legal (can be mitigated)
            'growth': 0.05     # Growth (can be learned)
        }
